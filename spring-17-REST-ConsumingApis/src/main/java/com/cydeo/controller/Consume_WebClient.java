...
public class Consume_WebClient {
    ...
    // I created end points with a reactive way using Mono and Flux.
    // After created end points, I consume these end points via webClient
    ...
    @GetMapping("/flux-movie-cinemas") // Consuming my own API. Flux is a stream which returns zero or more items (0…N).
    public Flux<MovieCinemaDTO> readAllCinemaFlux() { // using Flux, not using List, different structure which is called reactive programming
...
//    }
    ...
    @GetMapping("/mono-movie-cinema/{id}") // Mono is a stream which returns zero items or a single item (0…1).
...
//    ---------------------------WEBCLIENT---------------------------
    ...
    public Flux<MovieCinemaDTO> readWithWebClient() {
        ...
        return webClient // consume via webClient
                .get()
