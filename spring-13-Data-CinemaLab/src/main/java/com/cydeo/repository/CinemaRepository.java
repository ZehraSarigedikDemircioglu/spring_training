...
public interface CinemaRepository extends JpaRepository<Cinema, Long> {
    ...
    String readCinemaWithSpecificId(Long id);


    // ------------------- Native QUERIES ------------------- //

    //Write a native query to read all cinemas by location country
    @Query(value = "select * from cinema c join location l on c.location_id = l.country = ?1", nativeQuery = true)
    List<Cinema> readAllCinemaByCountry(String country);


    //Write a native query to read all cinemas by name or sponsored name contains a specific pattern
    @Query(value = "select * from cinema where name ilike %?1% or sponsored_name ilike %?1%", nativeQuery = true)
    List<Cinema> readAllCinemaByNameOrSponsoredNameContains(String pattern);
