...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> sortAllAccountWithAge();
    ...
    List<Account> retrieveAllAccountsAgeLessThan(int age);
    ...
    List<Account> retrieveAllAccountsContainsNameAddressCountryStateCity(@Param("pattern") String pattern);


    //Write a native query to read all accounts with an age higher than a specific value
    @Query(value = "select * from account_details where age > ?1", nativeQuery = true)
    List<Account> readAllAccountWithAgeGreater(int age);
}
